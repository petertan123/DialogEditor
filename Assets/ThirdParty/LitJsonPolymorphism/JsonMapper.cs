...
namespace LitJson
{
    ...
    public class JsonMapper
    {
                    ...
                    if (t_data.Properties.ContainsKey (property)) {
                        ...
                        if (t_data.IsDictionary)
                        {
                            ...
                            if (converter != null)
                            {
                                ...
                                //判断一下是否符合打上多态标签
                                if (TypeIsPolymorphismJsonAttribute(t_data.ElementType))
                                {
                                    reader.Read();//ObjectStart
                                    reader.Read();//__ASSEMBLY__
                                    reader.Read();
                                    ...
                                    Type t = Assembly.Load(assembly).GetType(type);
                                    reader.Token = JsonToken.ObjectStart;
                                    ((IDictionary)instance).Add(temp, ReadValue(t, reader, true)); // 这里第三个参数用来标记跳过读取操作
                                }
                                else
                                {
                                        ...
                                        int depth)
        {
            ...
            // MARKER:这段是摘自ET的LitJson的，与ILRuntime有关，普通项目中可以去掉
